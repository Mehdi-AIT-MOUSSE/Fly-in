...
class Parse:
    ...
    def get_clean_lines(self):
        try:
            with open(self.path) as file:
                lines = [s for line in file if (s := " ".join(line.strip().split())) and not s.startswith("#")]
        except Exception:
        ...
        lines = [line.replace('[ ', '[').replace(' ]', ']').replace(' =', '=').replace('= ', '=').replace(" :", ":") for line in lines]
    ...
    def get_nb_drones(self):
        ...
        nbs = list(filter(lambda l: l.startswith("nb_drones"), self.clean_lines))
    ...
    def verify_hub(self, line, is_start, is_end, names = set(), cords = set()):
        ...
        if len(parts) > 3:
            ...
            if not (metadata.startswith('[') and metadata.endswith(']')):
                raise ParseError("Metadata must be written between square brackets []")
            ...
            for meta in metadata:
                ...
                if key == "max_drones":
                    ...
                    if value not in allowed_colors:
                        raise ParseError("Only red, blue, green and yellow colors are available.")
    ...
    def get_connection(self):
        ...
        for line in connections:
            ...
            if len(line) == 0:
                raise ParseError("Empty connection follow this format  connection: <zone1>-<zone2> [metadata]")
            ...
            if len(line) > 1:
                ...
                if len(metadata) > 1:
                    raise ParseError("Connection metadata only supports max_link_capacity (max_link_capacity=int).")

                if (len(metadata) == 1):
                    metadata = metadata[0]
                    if "=" not in metadata:
                        raise ParseError("Metadata must be in the [key=value] format.")
                    
                    key, value = metadata.split("=")
                    if key != "max_link_capacity":
                        raise ParseError("The connection metadata allows only max_link_capacity (max_link_capacity=int).")
                    ...
                    try:
                        ...
                        raise ParseError("max_link_capacity must be positive integer.")
